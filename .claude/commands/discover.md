...
2. **Orients** from `state.json` → `MEMORY.md` → latest session log → `00-Setup/evidence-handling-terms.md`, then runs `cli.ts intake` — material waiting in `02-Workflow/evidence/` belongs in `/capture`, not here, and structuring registers while it sits unread is wasted work. **Hard stop if the terms are unsigned** — capture cannot be structured before handling is agreed.
...
5. **Writes one instrument per chunk.** It never assigns an id — ids are minted by code at accept time. A row that needs one goes through `/capture`.
