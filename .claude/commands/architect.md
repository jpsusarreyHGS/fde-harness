...
2. **Target architecture** — components, data flow, where the ontology, the agent and the human gates sit. Mermaid source into `03-Architecture/architecture-diagram.md` as the canonical file.
...
4. **Access model** — roles x objects x operations from `02-Design/personas.md`. Every write names an approver role.
...
Writes `03-Architecture/architecture.md`, `architecture-diagram.md` and `access-model.md`, and logs architecture decisions to `chronicle/memory/decisions.md` with the rejected alternative.
