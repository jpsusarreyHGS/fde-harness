...
3. **Assembles `02-Design/` in dependency order** — glossary → personas → competency questions → use cases → source systems. Each maps 1:1 onto the ontology repo's `docs/00-business/` layer.
...
5. **Logs every promotion** to `ontology-intake/promotion-log.md` with the `REQ-`/`EV-` ids behind it — this is what builds the traceability map and what you show a client who asks why an entity exists.
...
The agent never drafts an entity from a blank page. Anything with no requirement behind it is parked in `ontology-intake/ontology-backlog.md` with a note on what evidence would justify it.
